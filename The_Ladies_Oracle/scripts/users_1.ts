 ...
 *   npx tsx scripts/users.ts --delete-all --keep me@x.com --keep her@y.com [--yes]   # everyone except the kept ones
 *   … add --keep <uid-or-email> (repeatable) to protect an account whatever it's called,
...
async function main() {
  ...
  const keep = new Set(args.flatMap((a, i) => (a === '--keep' && args[i + 1] ? [args[i + 1].toLowerCase()] : [])));
  const deleteAll = args.includes('--delete-all');
  if (deleteAll && keep.size === 0) throw new Error('--delete-all needs at least one --keep <uid-or-email> (refusing to empty the database)');
  ...
  // ── --list ──
  if (needle === null && !oneUid && !deleteAll) {
    print(all);
  ...
  // ── select victims ──
  const isKept = (r: Row) => keep.has(r.uid.toLowerCase()) || (r.email && keep.has(r.email.toLowerCase()));
  const victims = all.filter(r => !isKept(r) && (deleteAll ? true : oneUid ? r.uid === oneUid : ((nameOnly ? r.name : `${r.name} ${r.email}`).toLowerCase().includes(needle!))));
  const kept = all.filter(isKept);
  if (deleteAll) { console.log(`Keeping ${kept.length}:`); print(kept); console.log(''); }
